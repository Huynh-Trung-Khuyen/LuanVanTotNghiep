...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $imported_bid_name = $_POST['imported_bid_name'];
    ...
    if (empty($imported_bid_name) || empty($quantity) || empty($input_day) || empty($expired_date) || empty($supplier_id) || empty($seri_number)) {
        $error = 'Không được để trống!';
    } else {
        $query = $conn->prepare('
            INSERT INTO warehouse_bid
            (imported_bid_name, quantity, input_day, expired_date, supplier_id, seri_number, purchase_price)  
            VALUES
            (:imported_bid_name, :quantity, :input_day, :expired_date, :supplier_id, :seri_number, :purchase_price)  
        ');
        $query->bindParam(':imported_bid_name', $imported_bid_name);
        ...
        <div class="content-wrapper">
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Thêm Sản Phẩm Vào Kho Phiên Đấu Giá</h1>
                        </div>
                    </div>
                </div>
            </div>
            <form action="#" method="POST" class="" role="form">
                <div class="card-body">
                    <div class="form-group">
                        <label for="imported_bid_name">Tên Sản Phẩm Nhập Kho</label>
                        <input type="text" name="imported_bid_name" class="form-control" placeholder="Nhập tên sản phẩm nhập kho">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="purchase_price" class="form-control" placeholder=".000vnđ">
                    </div>
                    <div class="form-group">
